...
import { confirmPurchasePayment } from "@/lib/payments/confirm";

// 藍新 NotifyURL 背景通知，Form Post 送达（application/x-www-form-urlencoded），
// 权威付款结果来源，见 .claude/specs/T3.1-T3.2-newebpay-spec.md 1.4 节。
export async function POST(request: NextRequest) {
  ...
  const result = await confirmPurchasePayment({ TradeInfo: tradeInfo, TradeSha: tradeSha });
  if (!result.ok) {
